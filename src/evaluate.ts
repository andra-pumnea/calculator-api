...
export function calculate(expression: string): string {
  let calculatorStates: CalculatorState = {
    operatorStack: [],
    valuesQueue: []
  };
...
function evaluteExpression(
  token: string,
  calculatorStates: CalculatorState
): CalculatorState {
  if (isNumber(token)) {
...
function evaluateParanthesisExpression(
  calculatorStates: CalculatorState
): CalculatorState {
...
function getPrecedence(
  op1: CalculatorOperation,
  op2: CalculatorOperation
): boolean {
